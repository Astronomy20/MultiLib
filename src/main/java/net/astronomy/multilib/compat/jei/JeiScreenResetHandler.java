...
/**
 * Resets every {@link MultiblockRecipeCategory.ViewState} (zoom, yaw, pitch, layer) whenever the
 * JEI recipes GUI screen is closed, so a freshly (re)opened JEI window always starts from defaults
 * while still preserving per-definition state across recipe/category switches within one open window.
 *
